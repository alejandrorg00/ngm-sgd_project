...
## Load data
class PermutableMNIST(datasets.MNIST):
    ...
    def perm(self, permutation):
        ...
        def _permute(img):
            ...
            permuted = arr[permutation]
            h, w = img.size[1], img.size[0]
            permuted_img = Image.fromarray(permuted.reshape(h, w))
            return permuted_img

        permute_transform = transforms.Lambda(_permute)
        new_transform = transforms.Compose([permute_transform] + base_ts)

        return PermutableMNIST(
            root=self.root,
            train=self.train,
...
# train dataset
train_ds = PermutableMNIST(root="./data",train=True, download=True, transform=base_transform)
# test dataset
test_ds = PermutableMNIST(root="./data", train=False, download=True, transform=base_transform)

perm_id = np.arange(28*28)
rng = np.random.RandomState(42)
perm_shuffle = rng.permutation(28*28)
perm_shuffle2 = rng.permutation(28*28)
...
def continual_train(model, optimizer, criterion,contexts, batch, ctx_iter, rho_eval,train_ds, test_ds, device, mode):
    ...
    for task_id, ctx in enumerate(contexts):
        print(f"\nContext {task_id+1}/{len(contexts)}")

        # --------------------- TRAIN LOADER --------------------------
        train_perms = [train_ds.perm(p) for p in ctx]
        train_subsets = ConcatDataset(train_perms)
        sampler = RandomSampler(train_subsets,replacement=True,num_samples=ctx_iter * batch)
        train_loader = DataLoader(train_subsets,batch_size=batch,sampler=sampler)
        train_iter = iter(train_loader)
        
        # --------------------- TEST LOADER ---------------------------
        test_perms = [test_ds.perm(p) for p in [perm_id]]
        test_subsets = ConcatDataset(test_perms)
        #eval_sampler = RandomSampler(test_subsets,replacement=False,num_samples=2000)
        #test_loader = DataLoader(test_subsets,batch_size=batch,sampler=eval_sampler)
        test_loader  = DataLoader(test_subsets, batch_size=batch, shuffle=False) # all dataset each time
...
rho_eval = 1
ctx_iter = 200
contexts = [[perm_id], [perm_id, perm_shuffle], [perm_id, perm_shuffle, perm_shuffle2]]
...
def run_all_models_nSims(nSims, base_seed=0):
    ...
    for sim in range(nSims):
        ...
        hgainModel = GainModNet(input_dim=28*28, hidden_dim=400, output_dim=10,
                                 g0=1.0, gamma=0.9, eta=0.3).to(device)

# -*- coding: utf-8 -*-
"""
Split CIFAR-10 Hyperparam lr sweep
"""
...
## Simulation
def run_all_models_nSims_lr(nSims, lr_list, base_seed=0):
    all_results = []

    for lr in lr_list:   # <<< extra loop for learning rates
        print(f"\n### Learning rate = {lr} ###")
        ...
        for sim in range(nSims):
            ...
            hgainOptimizer = optim.SGD([ 
                {'params': [p for n, p in hgainModel.named_parameters() if 'gains' not in n and not n.startswith('fc.')],'lr': lr,'momentum': 0.9},
                {'params': [p for n, p in hgainModel.named_parameters()if n == 'fc.weight'],'lr': lr,'momentum': 0.0} 
            ])
            ...
            adamOptimizer = optim.Adam(
                [{'params':[p for n,p in adamModel.named_parameters() if 'gains' not in n and n!='fc.bias'],'lr':lr}]
            )
            ...
            msgdOptimizer = optim.SGD(
                [{'params':[p for n,p in msgdModel.named_parameters() if 'gains' not in n and n!='fc.bias'],'lr':lr}], 
                momentum=0.9
            )
            ...
            # --- SGD ---
            print("SGD")
            sgdModel = GainSlimResNet18(output_dim=10,nf=20,g0=1.0, gamma=0.0, eta=0.0).to(device)
            sgdOptimizer = optim.SGD(
                [{'params':[p for n,p in sgdModel.named_parameters() if 'gains' not in n and n!='fc.bias'],'lr':lr}], 
                momentum=0.0
            )
            sgd_hist = continual_train(sgdModel, sgdOptimizer, criterion,
                                       contexts, batch, ctx_iter, rho_eval,
                                       train_ds, test_ds, device, mode="no-gain")
            ...
            all_results.append({"seed": seed, "lr": lr, "results": results})
...
all_sims = run_all_models_nSims_lr(nSims=5, lr_list=lr_list, base_seed=42)

# save results
import pickle
with open("cifar10_lr_methodsweep.pkl", "wb") as f:
    pickle.dump(all_sims, f)
print("Saved results to cifar10_lr_methodsweep.pkl")
